function [simdata, state, newPointIndex] = marinerPathWithEarlyStopping(wpt, R_switch, environmentRandomValues)
    ...
    for i=1:N+1
        ...
        if mod(i,CheckpointDistance) == 0
            if length(checkpointList) > numCheckPointsToCheck
                if all(diff(checkpointList(end-numCheckPointsToCheck:end)) > 0)
                    stopSimulation = true;
