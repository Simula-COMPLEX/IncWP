function [endIteration,reachedWaypoint, lengthOfPath, lastPoint, subpath, angles] = runSubPathRemus100(wpt, currentWaypointIndex, R_switch, environmentRandomValues,currentIterationNumber, prevIterationNumber, vesselResultsPath)
    numberOfSamples = size(environmentRandomValues,2); 
    if currentWaypointIndex == 2
        
        %% USER INPUTS
        h = 0.05;                           % Sampling time [s]
        N = numberOfSamples; %28000*2;                         % Number of samples to simulate
    
    
        % Initialize position and orientation
        xn = 0; yn = 0; zn = 0;             % Initial North-East-Down positions (m)
        phi = 0; theta = 0;                 % Initial Euler angles (radians)
        psi = atan2(wpt.pos.y(2) - wpt.pos.y(1),wpt.pos.x(2) - wpt.pos.x(1));   % Yaw angle towards next waypoint
    ...
    for i = startIteration:N+1
       ...
       if mod(i,CheckpointDistance) == 0
           if length(checkpointList) > numCheckPointsToCheck
               if all(checkpointList((end-numCheckPointsToCheck):end) < distanceToFinalWpt)
                   stopSimulation = true;
                   reachedWaypoint = false;
               end
           end
       ...
       if stopSimulation
            ...
            subpath = simdata(startIteration:i,18:20);
            angles = simdata(startIteration:i,21:23);
