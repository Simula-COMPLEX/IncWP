function effenctMetrics(vesselName)
    ...
    for wayPointIdx = waypointTimestampMap.keys()
        ...
        for selectionType = selectionTimeStampAndClassMap.keys()
            ...
            [mintime, maxtime] = bounds([missingTimestamps; stableTimestamps; unstableTimestamps]);
            ...
            if size(missingTimestamps, 1) > 0
                ...
                numberMissing = length(missingTimestamps);
            else
                firstMissing = 0;
                numberMissing = 0;
            end
            if size(unstableTimestamps, 1) > 0
                ...
                numberUnstable = length(unstableTimestamps);
            else
                firstUnstable = 0;
                numberUnstable = 0;
            end
            
            missingEffincency = numberMissing/timeduration; 
            unstableEffincency = numberUnstable/timeduration;
    ...
    for wayPointIdx = waypointTimestampMap.keys()
        ...
        selectionKeys = setdiff(selectionKeys, "FullWP");
            ...
            if ~(selectionType == "FullWP" )
                ...
                plot(ax(2), 1:length(unstableMinCount), unstableMinCount, ...
                            'DisplayName', selectionType, ...
                            'Color', currentColor, ...
                            'LineWidth', 1.5);
                plot(ax(3), 1:length(stableMinCount), stableMinCount, ...
                            'DisplayName', selectionType, ...
                            'Color', currentColor, ...
                            'LineWidth', 1.5);
