...
%   NotTd 	<protected>	the function called after each generation of the execution
...
%--------------------------------------------------------------------------
    ...
    function nofinish = NotTerminated(obj,Population)
        ...
        %       end
            ...
            %if length(obj.generationPerformances) > 10
                % check convergence
            %    nofinish = ~searchConverged(bj.generationPerformances((end-10):end))  
            %else
            %    nofinish = true;
            %end
